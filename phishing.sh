...
# Run the SDG phishing workload
JAR="/root/simple-data-generator/build/libs/simple-data-generator-1.0.0-SNAPSHOT.jar"
YAML="/root/simple-data-generator/secops-email.yml"

if [[ -f "$JAR" && -f "$YAML" ]]; then
  exec java -jar "$JAR" "$YAML"
else
  echo ":x: SDG JAR or YAML file not found. Check paths:"
  echo "JAR:  $JAR"
  echo "YAML: $YAML"
  exit 1
fi
